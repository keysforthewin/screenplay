...
describe('buildDialogContext', () => {
  it('includes the logline (title + synopsis)', async () => {
    ...
    const beat = await Plots.createBeat({ projectId, name: 'Open road', desc: 'd', body: 'b' });
  ...
  it('omits the previous-beat section for the first beat', async () => {
    const b1 = await Plots.createBeat({ projectId, name: 'First', desc: 'd', body: 'b' });
    const ctx = await buildDialogContext(projectId, b1);
    expect(ctx).not.toMatch(/previous beat/i);
  });

  it('includes the per-beat dialogue notes when present', async () => {
    const beat = await Plots.createBeat({ projectId, name: 'Tense', desc: 'd', body: 'b' });
  ...
  it('includes character bios for the beat speakers', async () => {
    ...
    const beat = await Plots.createBeat({
      projectId, name: 'Meet', desc: 'd', body: 'b', characters: ['Alice'],
    });
