...
beforeEach(async () => {
  ...
  it('re-expands every shot of the beat against the bible', async () => {
    ...
    await createBeat({ projectId, name: 'Bulk', desc: 'd', characters: [] });
    const beat = await getBeat(projectId, 'Bulk');
    await setBeatSceneBible(projectId, 'Bulk', { location: 'Diner' });
    await createStoryboard({ projectId, beatId: beat._id, order: 1, textPrompt: 'OLD1', summary: 'shot one', shotType: 'medium', durationSeconds: 4 });
    await createStoryboard({ projectId, beatId: beat._id, order: 2, textPrompt: 'OLD2', summary: 'shot two', shotType: 'close_up', durationSeconds: 3 });
  ...
  it('reports done with zero shots (no-op)', async () => {
    ...
    await createBeat({ projectId, name: 'Empty', desc: 'd' });
    const beat = await getBeat(projectId, 'Empty');
    const jobId = await gen.startReExpandAllJob({ projectId, beatId: beat._id.toString() });
    const job = await drain(jobId);
    expect(job.status).toBe('done');
