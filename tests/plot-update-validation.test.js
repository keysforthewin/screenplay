...
describe('updatePlot input validation', () => {
  it('throws when patch is a string (the model-passed-value-as-patch bug)', async () => {
    await expect(Plots.updatePlot(undefined, 'a long synopsis string')).rejects.toThrow(/must be an object/);
  });

  it('throws when patch is an array', async () => {
    await expect(Plots.updatePlot(undefined, ['synopsis', 'foo'])).rejects.toThrow(/must be an object.*array/);
  });

  it('throws when patch is null', async () => {
    await expect(Plots.updatePlot(undefined, null)).rejects.toThrow(/must be an object/);
  });
  ...
  it('valid patch.synopsis still updates end-to-end', async () => {
    await Plots.updatePlot(undefined, { synopsis: 'A great movie about debugging.' });
  ...
  it('updatePlot throws when the doc disappears mid-write', async () => {
    ...
    try {
      await expect(Plots.updatePlot(undefined, { synopsis: 'wont land' })).rejects.toThrow(
