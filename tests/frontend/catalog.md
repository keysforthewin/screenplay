...
filename regex (use `imageUrls + fileLinks` — Discord renders bot file uploads
as `<a>` file cards, not `<img>`), `readback` follow-up assertion, `capture`
extract id.

`runId` is a base-36 timestamp, e.g. `lwz9k3a`. Image URLs (use originals, not
`/thumb/...` — Wikimedia blocks automated thumbnail hotlinks regardless of UA):
...
| ID | Tool | Prompt | Verify |
|---|---|---|---|
| T1.1 | `update_character_template` (add) | Add a `tagline` string field to the character template. Optional, not core. | text: `/(updated\|added\|saved)/i` AND contains `tagline` |
...
| ID | Tool | Prompt | Verify |
|---|---|---|---|
| T2.1 | `update_plot` | Set the plot synopsis to: "A brittle prodigy and a charming wreck must rob a bank that stole their father." Notes: "draft 1, no act breaks yet". | text: `/(plot updated\|saved\|updated)/i` |
...
| T2.12 | `append_to_beat_body` | Append to the current beat's body: "He smiles. Cut to black." | text: `/appended/i` |
...
| T3.4 | `set_main_character_image` | Make image `${hero_img2}` the main image for `T_${runId}_Hero`. | text: `/main image/i` |
...
| T4.4 | `set_main_beat_image` | Make `${beat_img1}` the main image for the current beat. | text: `/main image/i` |
| T4.5 | `show_image` | Show me image `${beat_img1}`. | attach: (imageUrls.length + fileLinks.length) >= 1 |
...
| ID | Tool | Prompt | Verify |
|---|---|---|---|
| T5.1 | `generate_image` (library only) | Generate an image with the prompt: "moody noir alleyway, neon puddle reflections, 16:9 cinematic". Save to library — DON'T attach to any beat. | attach: (imageUrls.length + fileLinks.length) >= 1; capture `gen_img1` from a 24-hex id in text |
...
| T5.3 | `generate_image` (with beat) | Generate an image based on the current beat's content. Square aspect ratio. Attach it to the current beat. | attach: (imageUrls.length + fileLinks.length) >= 1 |
| T5.4 | `generate_image` (with chat context) | Generate an image inspired by the recent chat in this channel — wide aspect ratio, save to library only. | attach: (imageUrls.length + fileLinks.length) >= 1 |
...
| T6.5 | `tmdb_show_image` | Pull and display the Matrix poster from TMDB (the poster URL on image.tmdb.org). | attach: (imageUrls.length + fileLinks.length) >= 1 |
...
| T7.3 | `tavily_show_image` | Show this image: `URL_A` — caption "library cat". | attach: (imageUrls.length + fileLinks.length) >= 1 |
