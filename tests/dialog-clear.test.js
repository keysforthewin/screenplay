...
describe('deleteAllDialogsForBeatViaGateway', () => {
  it('removes every dialog for the beat and returns a count', async () => {
    ...
    await Dialogs.createDialog({ beatId: beat._id, body: 'one', character: 'Alice' });
    await Dialogs.createDialog({ beatId: beat._id, body: 'two', character: 'Bob' });
    await Dialogs.createDialog({ beatId: beat._id, body: 'three', character: 'Alice' });

    const result = await Gateway.deleteAllDialogsForBeatViaGateway({ beatId: beat._id });
  ...
  it('does not affect dialogs on other beats', async () => {
    ...
    await Dialogs.createDialog({ beatId: beatA._id, body: 'a1', character: 'Alice' });
    await Dialogs.createDialog({ beatId: beatB._id, body: 'b1', character: 'Bob' });
    await Dialogs.createDialog({ beatId: beatB._id, body: 'b2', character: 'Bob' });

    await Gateway.deleteAllDialogsForBeatViaGateway({ beatId: beatB._id });
