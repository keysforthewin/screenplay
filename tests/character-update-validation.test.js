...
describe('updateCharacter input validation', () => {
  it('throws when patch is a string (the model-passed-value-as-patch bug)', async () => {
    ...
    await expect(Characters.updateCharacter(undefined, c._id.toString(),'a long memes value')).rejects.toThrow(
      /must be an object/,
    );
  });
  ...
  it('throws when patch is an array', async () => {
    const c = await Characters.createCharacter({ name: 'Rae' });
    await expect(Characters.updateCharacter(undefined, c._id.toString(),['name', 'New'])).rejects.toThrow(
  ...
  it('throws when patch is null', async () => {
    const c = await Characters.createCharacter({ name: 'Rae' });
    await expect(Characters.updateCharacter(undefined, c._id.toString(),null)).rejects.toThrow(
      /must be an object/,
    );
  });
  ...
  it('throws when patch has no recognized fields', async () => {
    const c = await Characters.createCharacter({ name: 'Rae' });
    await expect(Characters.updateCharacter(undefined, c._id.toString(),{ foo: 'bar' })).rejects.toThrow(
  ...
  it('valid patch.name still updates end-to-end', async () => {
    const c = await Characters.createCharacter({ name: 'Rae' });
    const updated = await Characters.updateCharacter(undefined, c._id.toString(),{ name: 'Renamed' });
  ...
  it('valid patch.fields.memes still updates end-to-end', async () => {
    const c = await Characters.createCharacter({ name: 'Rae' });
    const updated = await Characters.updateCharacter(undefined, c._id.toString(),{
      fields: { memes: 'doge' },
    });
  ...
  it('updateCharacter throws when the doc disappears mid-write', async () => {
    ...
    try {
      await expect(
        Characters.updateCharacter(undefined, c._id.toString(),{ name: 'Newer' }),
...
describe('updateCharacter unset support', () => {
  ...
  it('rejects unset when not an array', async () => {
    const c = await Characters.createCharacter({ name: 'Rae' });
    await expect(
      Characters.updateCharacter(undefined, c._id.toString(),{ unset: 'memes' }),
  ...
  it('rejects unset entries that are not non-empty strings', async () => {
    const c = await Characters.createCharacter({ name: 'Rae' });
    await expect(
      Characters.updateCharacter(undefined, c._id.toString(),{ unset: ['ok', ''] }),
    ).rejects.toThrow(/`unset` entries must be non-empty strings/);
    await expect(
      Characters.updateCharacter(undefined, c._id.toString(),{ unset: [123] }),
    ).rejects.toThrow(/`unset` entries must be non-empty strings/);
  });
  ...
  it('rejects when unset is empty and no other recognized changes', async () => {
    const c = await Characters.createCharacter({ name: 'Rae' });
    await expect(
      Characters.updateCharacter(undefined, c._id.toString(),{ unset: [] }),
