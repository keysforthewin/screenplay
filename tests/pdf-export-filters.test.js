...
async function seedScene() {
  await Characters.createCharacter({ projectId, name: 'Alice', plays_self: true, own_voice: true, fields: { background_story: 'A baker.' } });
  await Characters.createCharacter({ projectId, name: 'Bob', plays_self: true, own_voice: true, fields: { background_story: 'A thief.' } });
  await Characters.createCharacter({ projectId, name: 'Steve', plays_self: true, own_voice: true, fields: { background_story: 'A driver.' } });
  await Plots.createBeat({ projectId, name: 'Heist Setup', desc: 'They plan to steal the gun.', characters: ['Alice', 'Steve'] });
  await Plots.createBeat({ projectId, name: 'The Heist', desc: 'They steal the gun from the vault.', characters: ['Alice', 'Bob', 'Steve'] });
  await Plots.createBeat({ projectId, name: 'Aftermath', desc: 'They lay low.', characters: ['Bob'] });
  await Plots.createBeat({ projectId, name: 'Diner Scene', desc: 'They argue over breakfast.', characters: ['Alice', 'Bob'] });
}
...
describe('exportToPdf filter modes', () => {
  it('full export (no filters) writes a PDF and returns its path', async () => {
    await seedScene();
    const result = await exportToPdf({ projectId, title: 'Full' });
  ...
  it('characters filter writes a PDF containing only the named character sheets', async () => {
    await seedScene();
    const filtered = await exportToPdf({ projectId, title: 'Two Chars', characters: ['Alice', 'Bob'] });
    expect(filtered.path).toMatch(/\.pdf$/);
    const filteredSize = (await fsp.stat(filtered.path)).size;
    const full = await exportToPdf({ projectId, title: 'Full' });
  ...
  it('characters filter is case-insensitive (matches by name_lower)', async () => {
    await seedScene();
    const result = await exportToPdf({ projectId, characters: ['alice', 'BOB'] });
    expect(result.path).toMatch(/\.pdf$/);
  });

  it('characters filter returns an error when any name is missing', async () => {
    await seedScene();
    const result = await exportToPdf({ projectId, characters: ['Alice', 'Nobody'] });
  ...
  it('beats_query writes a PDF and excludes non-matching beats', async () => {
    await seedScene();
    const result = await exportToPdf({ projectId, beats_query: 'gun' });
    expect(result.path).toMatch(/\.pdf$/);
    const stat = await fsp.stat(result.path);
    expect(stat.size).toBeGreaterThan(500);
  });
  ...
  it('beats_query returns an error when nothing matches', async () => {
    await seedScene();
    const result = await exportToPdf({ projectId, beats_query: 'xyzzy-no-match' });
  ...
  it('dossier_character writes a PDF for one character + their beats', async () => {
    await seedScene();
    const dossier = await exportToPdf({ projectId, dossier_character: 'Steve' });
    expect(dossier.path).toMatch(/\.pdf$/);
    const dossierSize = (await fsp.stat(dossier.path)).size;
    const full = await exportToPdf({ projectId,});
    const fullSize = (await fsp.stat(full.path)).size;
  ...
  it('dossier_character matching is case-insensitive against beat.characters', async () => {
    await Characters.createCharacter({ projectId, name: 'Steve', plays_self: true, own_voice: true });
    // Beat stores the name with mixed case; lookup uses a different case.
    await Plots.createBeat({ projectId, name: 'Mixed', desc: 'd', characters: ['STEVE'] });
    const result = await exportToPdf({ projectId, dossier_character: 'steve' });
    expect(result.path).toMatch(/\.pdf$/);
  });

  it('dossier_character returns an error when the character does not exist', async () => {
    await seedScene();
    const result = await exportToPdf({ projectId, dossier_character: 'Ghost' });
...
describe('export_pdf handler precedence', () => {
  ...
  it('treats an empty characters array as "no filter" (does not flag precedence)', async () => {
    ...
    const result = await HANDLERS.export_pdf({ characters: [], beats_query: 'gun' }, { projectId });
