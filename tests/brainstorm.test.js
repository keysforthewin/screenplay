...
describe('rapid-fire brainstorm: stub character + beats + rename', () => {
  it('stub character can be linked at beat-create time and survives a later rename, with documented drift', async () => {
    ...
    const kid = await Characters.createCharacter({ projectId, name: 'Streamer Kid' });
    ...
    const renamed = await Characters.updateCharacter(projectId, kid._id.toString(),{ name: 'Marcus' });
    ...
    const beatStaleLink = await Plots.getBeat(projectId, beat._id.toString());
    ...
    await Plots.updateBeat(projectId, beat._id.toString(), { characters: ['Nully', 'Marcus'] });
    const beatFixed = await Plots.getBeat(projectId, beat._id.toString());
  ...
  it('parallel-creation shape: characters then beats with link arg in one logical batch lands intact', async () => {
    ...
    await Characters.createCharacter({ projectId, name: 'Nully' });
    await Characters.createCharacter({ projectId, name: 'Streamer Kid' });
    const b1 = await Plots.createBeat({ projectId,
      name: 'Nully Despawns Base',
      desc: 'The time Nully despawned the base.',
      characters: ['Nully'],
    });
    const b2 = await Plots.createBeat({ projectId,
      name: 'Kid Shoots Nully',
      desc: 'The wipe where a streamer kid shot Nully.',
      characters: ['Nully', 'Streamer Kid'],
    });

    const all = await Plots.listBeats(projectId);
  ...
  it('character search by name still works after rename (sanity for stub-then-rename flow)', async () => {
    await Characters.createCharacter({ projectId, name: 'Streamer Kid' });
    const got1 = await Characters.getCharacter(projectId, 'streamer kid');
    expect(got1).toBeTruthy();

    await Characters.updateCharacter(projectId, 'Streamer Kid', { name: 'Marcus' });
    const got2 = await Characters.getCharacter(projectId, 'marcus');
    expect(got2).toBeTruthy();
    expect(got2.name).toBe('Marcus');

    // Old name no longer resolves — good, it's been renamed.
    const got3 = await Characters.getCharacter(projectId, 'streamer kid');
