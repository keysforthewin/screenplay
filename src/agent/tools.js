  ...
  },
  {
    ...
    input_schema: {
      ...
      properties: {
        ...
        provider: { type: 'string', enum: ['nano-banana-pro', 'flux-2-pro', 'flux-pro-kontext', 'openai'], description: 'Image model provider. Default "nano-banana-pro" for text-only / single-reference, auto-bumped to "flux-2-pro" when 2+ reference images are passed. Pass "flux-2-pro" or "flux-pro-kontext" explicitly when the user names Flux. Pass "openai" (gpt-image-2) only when the user explicitly asks for OpenAI, GPT-image, or DALL-E.' },
        ...
        aspect_ratio: { type: 'string', enum: ['1:1', '16:9', '9:16', '4:3', '3:4'], description: 'Optional aspect ratio. Defaults to 16:9. For OpenAI, this maps to the closest supported gpt-image-2 size.' },
        source_image_id: { type: 'string', description: 'Optional 24-hex GridFS image id. When set, the image is loaded and passed to the chosen provider as a reference (img2img). The source is not modified. For combining multiple sources, use source_image_ids instead.' },
        source_image_ids: { type: 'array', items: { type: 'string' }, description: 'Optional array of 24-hex GridFS image ids (up to 9) to combine / mix / blend into a single new image. Use this for "put this character in this scene", "blend these two", "this outfit on this person". When 2+ ids are passed and no provider is set, the tool auto-selects flux-2-pro (best at blending references). None of the sources are modified.' },
  ...
  },
  {
    ...
    input_schema: {
      ...
      properties: {
        ...
        replace_source: { type: 'boolean', description: 'REQUIRED. true = delete the source image after a successful edit (truly "in place"); false = keep the source alongside the new image so the user can compare or revert. Only affects the primary source_image_id; additional_source_image_ids are never deleted.' },
