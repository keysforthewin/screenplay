...
export const STILL_FRAMING_RULES = [
  ...
  `- State WHERE the subject sits in the geography the beat requires — including the exact sub-location the beat or a mini-slug names (the back seat vs the front, at the head of the table, in the doorway, in its travel lane to one side of the centerline). If the beat puts the subject in the back seat, the still must read as the rear bench, never the front. This is shot-specific blocking — it is NOT a restatement of the scene bible.`,
