...
| `ANTHROPIC_MODEL` | optional | Defaults to `claude-fable-5-1`. Creative writing, storyboard planning and every synchronous generation pass. | Choosing a smarter or cheaper creative model. |
| `ANTHROPIC_AGENT_MODEL` | optional | Defaults to `claude-sonnet-5`. Runs the Discord agent loop; when it differs from `ANTHROPIC_MODEL` the loop delegates writing to a subagent on the creative model. Set equal to `ANTHROPIC_MODEL` for single-model mode. | Trading orchestrator cost against capability. |
| `ANTHROPIC_ENHANCER_MODEL` | optional | Defaults to `claude-fable-5-1`. Short auxiliary passes: prompt enhancement, image captioning, reference scoring, PDF filenames. | Making the cheap passes cheaper. |
